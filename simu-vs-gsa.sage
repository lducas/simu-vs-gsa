...
l = [log(bkzgsa_gso_len(0, i, d, delta=delta)) / log(2) for i in range(d)]    

b0 = 2**l[0]
print(beta, "\t", b0, "\t", b0)

for beta in range(3, 512):
    l = simBKZ(l, beta)
    b0_simu = 2**l[0]
    b0_gsa = bkzgsa_gso_len(0, 0, d, delta=compute_delta(beta))
    print(beta, "\t", b0_simu, "\t", b0_gsa)